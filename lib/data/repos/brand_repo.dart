...
try {
    ...
    final documentsQuery = await _db.collection('BrandCategories').where('categoryId', isEqualTo: categoryId).get();
    // Extract the brands ids from the documents
    List<String> brandIds = documentsQuery.docs.map((doc) => doc['brandId'] as String ).toList();
    ...
    final snapshot = await _db.collection('Brands').where(FieldPath.documentId, whereIn: brandIds).limit(2).get();
    final brands = snapshot.docs.map((document) {
      return BrandModel.fromQuerySnapshot(document);
    }).toList();
    return brands ;
